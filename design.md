...
- **CanvasRenderer**（`presentation/CanvasRenderer.ts`）: `GameState` を受け取り pointy-top hex を Canvas 2D に描画。
  ...
  **選択中ユニット（自/敵いずれも）はマップ上でも強調リングを描く**。
  敵相プレビューとプレイヤー相内の攻撃履歴は、攻撃元から攻撃先への矢印と `-damage` ラベルで表示する。
  同一対象が複数のユニットから攻撃される場合も各攻撃元からの矢印をすべて残し、右下パネルの HP 減少は合計値として表示する。
  敵相で同一対象が複数の敵から攻撃される場合のみ、対象上にも合計 `-damage` を併記する。相互攻撃の矢印は互い違いのレーンで描く。
...
- **右下：可視敵ユニット一覧**（`visible` タイル上の敵のみ）
  - 各行：ID、現在 HP。**敵相では敵は被弾しない**（MVP では敵 HP はプレイヤーの即時攻撃でのみ減る）ため、
    **ターン終了時 HP 遷移は 0**（将来ロボットの自律攻撃等で非 0 になり得る。`tbd.md` G-11）。
    ただしプレイヤー相内の攻撃履歴が Undo 可能な状態で残っている場合、対象敵には `HP before/max -> after/max` として
    攻撃による HP 遷移を明示する。同一敵への複数攻撃では減少値を合計し、Undo では最後に指示した攻撃から逆順に矢印と
    HP 遷移を戻す。`after` が 0 の場合は state から敵が除去済みでも右下に行を残し、赤系強調と `Down this phase` で
    プレイヤー相中に倒れる予定を示す。
