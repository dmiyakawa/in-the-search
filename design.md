...
- **右下：可視敵ユニット一覧**（`visible` タイル上の敵のみ）
  ...
  - 各行はクリックで該当敵を `enemy` 選択にし、マップとパネルで強調する。逆に**マップ上の可視敵をクリック**したら、
    右下一覧の該当行が強調される（双方向連動）。
...
2. クリック先に**可視の敵ユニット**がいる → `enemy` 選択（右下＋マップ強調）。
3. `own` 選択中で、対象が**隣接の可視敵/巣** → 攻撃（`AttackUnit`）。
