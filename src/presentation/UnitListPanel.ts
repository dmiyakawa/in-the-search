...
export const renderUnitList = (
  ...
  for (const unit of state.units.filter(isPlayerSide)) {
    ...
    const doomed = prediction !== undefined && unit.hp - damage <= 0;
    ...
    if (doomed) row.classList.add('doomed');
    ...
    row.innerHTML = `
      ...
      ${doomed ? '<span class="row-alert">Down this enemy phase</span>' : ''}
  ...
  const podDoomed = prediction !== undefined && state.pod.hp - podDamage <= 0;
  const podRow = document.createElement('div');
  podRow.className = 'panel-row pod-row';
  if (podDoomed) podRow.classList.add('doomed');
  podRow.innerHTML = `
    ...
    ${podDoomed ? '<span class="row-alert">Down this enemy phase</span>' : ''}
  `;
  root.append(podRow);
};
