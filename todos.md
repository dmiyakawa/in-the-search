...
- 2026-06-21: タスク `20260621_018` として、プレイヤー側攻撃矢印と HP 減少表示を上方向へ持ち上げ、
  敵側攻撃予告矢印との重なりを軽減した。
- 2026-06-21: タスク `20260621_019` として、直近プレイヤー攻撃後の敵パネルに `HP before/max -> after/max` を表示し、
  Undo 時に攻撃矢印・HP 遷移表示が残らないよう修正した。
- 2026-06-21: タスク `20260621_020` として、自相攻撃で倒れる敵を即時に画面から消えたままにせず、Canvas では X 印、
  右下パネルでは `HP before/max -> 0/max` と `Down this phase` で撃破予定を残すようにした。敵相で倒れる自ユニット/Pod も
  DOWN 行を赤系強調する。
- 2026-06-21: タスク `20260621_021` として、プレイヤー相内の攻撃表示を単一の直近攻撃ではなく攻撃履歴として保持し、
  複数ユニットから同一敵への矢印をすべて描画、右下 HP 減少は合計表示、Undo は指示と逆順で 1 攻撃ずつ表示を巻き戻すようにした。
- 確定済み方針（指示者判断）:
  - **UI は素の TS + DOM を継続**（フレームワーク非導入・D-17）。presentation を小モジュールに分割する。
  - **操作はハイブリッド**（design.md §12.5）。隣接の有効マスは移動/攻撃、行動対象でない空マスのクリックで選択解除。
- 直近の未コミット: タスク `20260621_021` の実装差分が未コミット。作業記録は
  `logs/20260621_021_multi_attack_undo.result.md`。
